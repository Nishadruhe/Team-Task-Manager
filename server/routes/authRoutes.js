...
router.post('/signup', async (req, res) => {
  try {
    ...
    const result = await run('INSERT INTO users (name, email, password, avatar_color) VALUES ($1, $2, $3, $4)', [name, email, hash, color]);
...
router.get('/me', auth, async (req, res) => {
  try {
    const user = await get('SELECT id, name, email, avatar_color, created_at FROM users WHERE id = $1', [req.user.id]);
